...
- Sovereignty-clean: the list is self-authored like the voice — keel never ships one (the porn drogue's prefilled blocklist stays the lone, explicitly-consented exception). Naming for strangers: "vices" is the operator's register; neutral default label could be "watchlist" (presets principle). Relates to [[2026-06-05-keel-areas-of-compulsion-via-zenborg]] — areas could eventually *feed* the list.
